...
def render_map(result: Dict, active_rows: List[Dict], depot: Dict, rank: int = 1):
    ...
    if v_lat:
        # Red circle markers
        fig.add_trace(go.Scattermapbox(lat=v_lat, lon=v_lon, mode="markers",
            marker=dict(size=28, color="#ef4444"),
            name="Visit order", hovertemplate=v_hov, showlegend=False))
        # White visit-order numbers on top (separate text trace)
        fig.add_trace(go.Scattermapbox(lat=v_lat, lon=v_lon, mode="markers+text",
            marker=dict(size=28, color="rgba(0,0,0,0)"),
            text=v_txt, textposition="middle center",
