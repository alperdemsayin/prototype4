...
def render_map(result: Dict, active_rows: List[Dict], depot: Dict):
    ...
    if v_lat:
        fig.add_trace(go.Scattermapbox(lat=v_lat, lon=v_lon, mode="markers+text",
            marker=dict(size=24, color="#ef4444"), text=v_txt,
            textposition="middle center", textfont=dict(size=14, color="#ffffff"),
            name="Visit order", hovertemplate=v_hov, showlegend=False))
...
def render_one_solution(result: Dict, active_rows: List[Dict], depot: Dict, rank: int = 1):
    ...
    with t1:
        render_map(result, active_rows, depot)
    ...
    with t2:
        ...
        st.download_button("Download deliveries CSV",
            df.to_csv(index=False).encode("utf-8"), f"deliveries_sol{rank}.csv", "text/csv")
    ...
    with t3:
        ...
        st.download_button("Download result JSON", build_bundle(result),
            f"mirp_result_sol{rank}.json", "application/json")
