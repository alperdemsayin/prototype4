...
def _make_map_trace(**kwargs):
    scatter_map = getattr(go, "Scattermap", None)
    if scatter_map is not None:
        return scatter_map(**kwargs)
    return go.Scattermapbox(**kwargs)


def _update_map_layout(fig: go.Figure, latitudes: List[float], longitudes: List[float]):
    center_lat = sum(latitudes) / len(latitudes) if latitudes else 0.0
    center_lon = sum(longitudes) / len(longitudes) if longitudes else 0.0
    common = dict(margin=dict(l=0, r=0, t=0, b=0), height=520)

    if getattr(go, "Scattermap", None) is not None:
        fig.update_layout(
            map=dict(style="open-street-map", center=dict(lat=center_lat, lon=center_lon), zoom=4.3),
            **common,
        )
    else:
        fig.update_layout(
            mapbox=dict(style="open-street-map", center=dict(lat=center_lat, lon=center_lon), zoom=4.3),
            **common,
        )
...
def render_map(result: Dict, active_rows: List[Dict], depot: Dict, rank: int = 1):
    ...
    fig = go.Figure()
    fig.add_trace(_make_map_trace(
        lat=[depot["lat"]], lon=[depot["lon"]],
        mode="markers+text",
        marker=dict(size=18, color="#1d4ed8", line=dict(width=2, color="#ffffff")),
    ...
    for row in active_rows:
        ...
        hover += "<extra></extra>"
        if row["name"] in visit_order:
            v_lat.append(row["lat"])
            v_lon.append(row["lon"])
            v_hov.append(hover)
            v_txt.append(str(visit_order[row["name"]]))
        else:
            uv_lat.append(row["lat"])
            uv_lon.append(row["lon"])
            uv_hov.append(hover)
    ...
    if uv_lat:
        fig.add_trace(_make_map_trace(
            ...
            lon=uv_lon,
            mode="markers",
            marker=dict(size=14, color="#f59e0b", line=dict(width=1.5, color="#ffffff")),
            name="Plants (unvisited)",
            hovertemplate=uv_hov,
            showlegend=False,
        ))
    if v_lat:
        fig.add_trace(_make_map_trace(
            ...
            lon=v_lon,
            mode="markers+text",
            marker=dict(size=28, color="#ef4444", line=dict(width=2, color="#ffffff")),
            text=v_txt,
    ...
    _update_map_layout(fig, all_lats, all_lons)
